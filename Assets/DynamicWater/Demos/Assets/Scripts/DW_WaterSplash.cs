...
public class DW_WaterSplash : MonoBehaviour {
    ...
    private void SpawnSplash(ParticleSystem splashPrefab, Vector3 position) {
        ...
        if (SplashSounds.Length > 0) {
			AudioSource.PlayClipAtPoint(SplashSounds[Random.Range(0, SplashSounds.Length)], position, 1f/(1+((GameObject.Find("Main Camera").transform.position - transform.position).magnitude)));
        }
    }
